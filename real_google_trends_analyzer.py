...
class RealGoogleTrendsAnalyzer:
                ...
                for i in range(0, len(keywords), 5):
                    ...
                    try:
                        self.pytrends.build_payload(
                            batch, timeframe='now 7-d', geo='SE')
    ...
    def get_product_trends(self, product_keywords):
        ...
        try:
            ...
            for i in range(0, len(product_keywords), 5):
                ...
                try:
                    self.pytrends.build_payload(
                        batch, timeframe='today 3-m', geo='SE')
                    interest_df = self.pytrends.interest_over_time()

                    if not interest_df.empty:
                        for product in batch:
                            if product in interest_df.columns:
                                recent_interest = interest_df[product].tail(
                                    7).mean()
    ...
    def get_seasonal_insights(self):
        ...
        current_keywords = seasonal_keywords.get(
            current_month, ['mode', 'elektronik', 'hem'])
    ...
    def _get_fallback_trends(self):
        ...
        return [
            {'keyword': 'iphone 15', 'category': 'elektronik', 'interest_score': 85,
                'trend_direction': 'up', 'search_volume': '50K-100K'},
            {'keyword': 'air fryer', 'category': 'hem', 'interest_score': 72,
                'trend_direction': 'stable', 'search_volume': '10K-50K'},
            {'keyword': 'gaming headset', 'category': 'elektronik',
                'interest_score': 68, 'trend_direction': 'up', 'search_volume': '10K-50K'},
            {'keyword': 'yoga mat', 'category': 'sport', 'interest_score': 55,
                'trend_direction': 'stable', 'search_volume': '1K-10K'},
            {'keyword': 'skincare routine', 'category': 'hälsa', 'interest_score': 45,
                'trend_direction': 'up', 'search_volume': '1K-10K'}
        ]

    def _get_fallback_product_trends(self):
        """Fallback product trends if API fails"""
        return [
            {'product': 'wireless earbuds', 'current_interest': 75,
                'trend_direction': 'up', 'recommendation': 'high_priority'},
            {'product': 'smart watch', 'current_interest': 65,
                'trend_direction': 'stable', 'recommendation': 'medium_priority'},
            {'product': 'robot vacuum', 'current_interest': 58,
                'trend_direction': 'up', 'recommendation': 'medium_priority'}
        ]
...
def main():
    ...
    for trend in trends[:5]:  # Show top 5
        print(
            f"  🔥 {trend['keyword']} - {trend['category']} - Score: {trend['interest_score']} ({trend['trend_direction']})")

    # Analyze specific products
    print("\n📊 Analyzing product trends...")
    product_keywords = ['iphone', 'samsung galaxy',
                        'airpods', 'nintendo switch', 'ps5']
    product_trends = analyzer.get_product_trends(product_keywords)

    print(f"\n✅ Product trend analysis:")
    for product in product_trends[:3]:  # Show top 3
        print(
            f"  📱 {product['product']} - Interest: {product['current_interest']} - {product['recommendation']}")
