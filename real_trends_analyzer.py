...
class RealTrendsAnalyzer:
    ...
    def get_trending_products(self, categories=['electronics', 'fashion', 'home']):
        ...
        try:
            ...
            for keyword in product_keywords[:5]:

                try:
                    # Build payload for this keyword
                    self.pytrends.build_payload(
                        [keyword], timeframe='today 12-m', geo=self.geo)
                    ...
                    if not interest_over_time.empty:
                        ...
                        recent_avg = interest_over_time[keyword].tail(
                            4).mean()  # Last 4 weeks
                        older_avg = interest_over_time[keyword].head(
                            8).mean()   # First 8 weeks
                        ...
                        if keyword in related_queries and related_queries[keyword]['rising'] is not None:
                            rising_queries = related_queries[keyword]['rising']['query'].head(
                                3).tolist()
                    ...
                    self.logger.warning(
                        f"Could not analyze keyword '{keyword}': {e}")
                    continue
    ...
    def _assess_affiliate_potential(self, keyword, trend_score, growth_rate):
        ...
        high_potential_terms = ['köp', 'bäst',
                                'test', 'pris', 'billig', 'erbjudande']
    ...
    def get_seasonal_trends(self, keyword, timeframe='today 12-m'):
        ...
        try:
            ...
            if not interest_over_time.empty:
                ...
                monthly_data = interest_over_time.groupby(
                    interest_over_time.index.month)[keyword].mean()
            ...
            self.logger.error(
                f"Error getting seasonal trends for '{keyword}': {e}")
...
if __name__ == "__main__":
    ...
    for trend in trending:
        print(
            f"📈 {trend['keyword']}: {trend['trend_score']} ({trend['growth_rate']})")
    ...
    if seasonal:
        ...
        print(
            f"   Current season: {'PEAK' if seasonal['is_peak_season'] else 'NORMAL'}")
