...
class ProductImporter:
    def __init__(self):
        self.amazon_associate_tag = os.getenv(
    ...
    async def import_trending_products(self, trending_keywords: List[str]) -> Dict:
        ...
        for keyword in trending_keywords:
            ...
            amazon_products = self.create_mock_products(
                keyword, 'amazon', category, 3)
            aliexpress_products = self.create_mock_products(
                keyword, 'aliexpress', category, 2)
            ...
            import_stats['by_platform']['aliexpress'] += len(
                aliexpress_products)
            import_stats['by_category'][category] = import_stats['by_category'].get(
                category, 0) + len(amazon_products) + len(aliexpress_products)
    ...
    def determine_category(self, keyword: str) -> str:
        ...
        gaming_keywords = ['gaming', 'spel', 'controller',
                           'konsol', 'headset', 'rgb', 'chair']
        elektronik_keywords = ['bluetooth', 'wireless',
                               'usb', 'charger', 'cable', 'speaker', 'headphones']
        sport_keywords = ['fitness', 'yoga',
                          'sport', 'träning', 'gym', 'löpning']
        hem_keywords = ['kitchen', 'kök', 'hem',
                        'home', 'storage', 'förvaring', 'cleaning']
    ...
    async def update_affiliate_suggestions(self, new_products: List[Dict]):
        ...
        print(
            f"✅ affiliate_suggestions.json uppdaterad med {len(new_products)} nya produkter")
